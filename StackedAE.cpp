...
{
    double mu=0.9;
    V_theta1=mu*V_theta1-alpha*theta1Grad;
    V_theta2=mu*V_theta2-alpha*theta2Grad;
    V_B1=mu*V_B1-alpha*b1Grad;
    V_B2=mu*V_B2-alpha*b2Grad;
    V_softMaxtheta=mu*V_softMaxtheta-alpha*softmaxThetaGrad;
    aeTheta1 += V_theta1;
    aeTheta2 += V_theta2;
    aeB1 += V_B1;
    aeB2 += V_B2;
 ...
 void StackedAE::ADA_updateParameters(MatrixXd &theta1Grad,MatrixXd &theta2Grad,MatrixXd &b1Grad,MatrixXd &b2Grad,MatrixXd &softmaxThetaGrad,double alpha){
     //
     G_theta1+=powMat(theta1Grad,2);
     MatrixXd tmp1=(sqrtMat(G_theta1).array()+1).matrix();
     aeTheta1-=alpha*reciprocal(tmp1).cwiseProduct(theta1Grad);
     //
     G_theta2+=powMat(theta2Grad,2);
     MatrixXd tmp2=(sqrtMat(G_theta2).array()+1).matrix();
     aeTheta2-=alpha*reciprocal(tmp2).cwiseProduct(theta2Grad);
     //
     G_B1+=powMat(b1Grad,2);
     MatrixXd tmpb1=(sqrtMat( G_B1).array()+1).matrix();
     aeB1-=alpha*reciprocal(tmpb1).cwiseProduct(b1Grad);
     //
     G_B2+=powMat(b2Grad,2);
     MatrixXd tmpb2=(sqrtMat( G_B2).array()+1).matrix();
     aeB2-=alpha*reciprocal(tmpb2).cwiseProduct(b2Grad);
     //
     G_softMaxtheta+=powMat(softmaxThetaGrad,2);
     MatrixXd tmptheta=(sqrtMat( G_softMaxtheta).array()+1).matrix();
     softMaxTheta-=alpha*reciprocal(tmptheta).cwiseProduct(softmaxThetaGrad);
 }
...
double StackedAE::NAG_computeCost(MatrixXd &theta1Grad,
        MatrixXd &b1Grad,MatrixXd &theta2Grad,
        MatrixXd &b2Grad,MatrixXd &softmaxThetaGrad,
        MatrixXd &data,MatrixXi &labels,double lambda)
{
   double mu=0.9;
   MatrixXd tmp1=aeTheta1+mu*V_theta1;
   MatrixXd tmp2=aeTheta2+mu*V_theta2;
   MatrixXd tmpb1=aeB1+mu*V_B1;
   MatrixXd tmpb2=aeB2+mu*V_B2;
   MatrixXd tmptheta=softMaxTheta+mu*V_softMaxtheta;
   MatrixXd groundTruth = binaryCols(labels,numClasses);
   int M = labels.rows();
   //forward calculate
   MatrixXd term1 = tmp1 * data;
   MatrixXd z2 = bsxfunPlus(term1,tmpb1);
   MatrixXd a2 = sigmoid(z2);
   MatrixXd term2 = tmp2 * a2;
   MatrixXd z3 = bsxfunPlus(term2,tmpb2);
   MatrixXd a3 = sigmoid(z3);
   MatrixXd z4 = tmptheta * a3;
   MatrixXd a4 = expMat(z4);
   MatrixXd a4ColSum = a4.colwise().sum();
   a4 = bsxfunRDivide(a4,a4ColSum);
   //calculate delta
   MatrixXd delta4 = a4 - groundTruth;
   MatrixXd delta3 = (tmptheta.transpose() * delta4).cwiseProduct(sigmoidGradient(z3));
   MatrixXd delta2 = (tmp2.transpose() * delta3).cwiseProduct(sigmoidGradient(z2));

   //calculate delta
   softmaxThetaGrad = (groundTruth - a4) * a3.transpose() * (-1.0 / M) +tmptheta * lambda;

   theta2Grad = delta3 * a2.transpose() * (1.0 / M) + tmp2 * lambda;
   b2Grad = delta3.rowwise().sum() * (1.0 / M);
   theta1Grad = delta2 * data.transpose() * (1.0 / M) + tmp1 * lambda;
   b1Grad = delta2.rowwise().sum() * (1.0 / M);

   //compute cost
   double cost = (-1.0 / M) * (groundTruth.cwiseProduct(logMat(a4))).array().sum()
       + lambda / 2.0 * tmptheta.array().square().sum()
       + lambda / 2.0 * tmp1.array().square().sum()
       + lambda / 2.0 * tmp2.array().square().sum();
                   ...
                   double lambda,double alpha,int maxIter,int batchSize)
{
    ...
    std::ofstream f("NGA_Cifar.txt",std::ofstream::out);
    ...
    for(int i = 0; i < maxIter;i++)
    {
        ...
        for(int j = 0;j < numBatches; j++)
        {
            if(j==0){
                ...
                G_theta1.setZero(theta1Grad.rows(),theta1Grad.cols());
                G_theta2.setZero(theta2Grad.rows(),theta2Grad.cols());
                G_B1.setZero(b1Grad.rows(),b1Grad.cols());
                G_B2.setZero(b2Grad.rows(),b2Grad.cols());
                G_softMaxtheta.setZero(softmaxThetaGrad.rows(),softmaxThetaGrad.cols());
            }
            miniTrainData = data.middleCols(j * batchSize,batchSize);
            miniLabels = labels.middleRows(j * batchSize,batchSize);
            //J += computeCost(theta1Grad,b1Grad,theta2Grad,b2Grad,softmaxThetaGrad,miniTrainData,miniLabels,lambda);
            J += NAG_computeCost(theta1Grad,b1Grad,theta2Grad,b2Grad,softmaxThetaGrad,miniTrainData,miniLabels,lambda);
           ...
           // ADA_updateParameters(theta1Grad,theta2Grad,b1Grad,b2Grad,softmaxThetaGrad,alpha);
        }
        J = J / numBatches;
